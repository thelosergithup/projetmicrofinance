...
function inserer_utilisateur($reg, $con) {
    ...
    if (($reg != NULL) && isset($reg["1"])) {
        ...
        if ($row == 0) {
            $nomP = addslashes($reg["1"]['nomuser']);
            $prenomP = addslashes($reg["1"]['prenomuser']);
            $log = NULL;
            if ($reg["1"]['photo'] != NULL) {
...
// On peut valider le fichier et le stocker définitivement
            ...
            $con->query("insert into utilisateur(nomuser, prenomuser, teluser, ligneuser, emailuser, genreuser, roleuser, "
 ...
 function lister_utilisateur($con) {
    ...
    while ($recuser = mysqli_fetch_assoc($rsltuser)) {
        ...
        <tr >
            ...
            <td><?php print nl2br(htmlspecialchars($recuser["etatuser"])) ?></td>
...
function update_utilisateur_profil($reg, $con) {
    ...
    $url = basename($path);
    if (($reg != NULL) && isset($reg["1"])) {
        $req = $con->query("select * from utilisateur where loginuser='".$reg["1"]['logine']."' and motdepasseuser='" . sha1($reg["1"]['pass1e']) . "'") or die(mysqli_error($con));
        $row = mysqli_num_rows($req);
        $nomP = addslashes($reg["1"]['nome']);
        $prenomP = addslashes($reg["1"]['prenome']);
        $pa = NULL;
